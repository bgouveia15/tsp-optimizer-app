// CUSTOM STARTUP FILE — v56 §3 (audit T-27).
//
// WHAT THIS IS FOR, in one sentence: without it, the app asks Google for
// a font whenever it meets a character none of its own fonts contain.
//
// That request carries nothing about the member, but it can fire while
// they are looking at their pay statement, and the character that
// triggers it can come from the statement itself — an unusual letter in
// a name read off an imported document. It is the third instance of the
// same defect in this project: something outside `lib/` reaching a third
// party, invisible to any reading of our own code because none of it is
// our code. (The Google graphics renderer was the first; the vendored
// text-recognition library's CDN defaults were the second.)
//
// `fontFallbackBaseUrl` below overrides that. The engine reads it from
// this config and only falls back to Google's font host when it is null
// — so setting it makes that address unreachable rather than merely
// unused. The string still appears in the compiled output as the dead
// default; its presence there proves nothing either way.
//
// (The Google host is deliberately NOT written out here. The deploy's
// foreign-origin gate reads this file and flags any off-origin address
// in it, and it caught this comment on its first run — correctly. A
// comment is not worth an allowlist entry that would also wave through
// a real one.)
//
// WHICH OUTCOME THIS IS. Pointing at our own origin does not put fallback
// fonts there. We serve none, so an unusual glyph renders as an empty box
// instead of fetching a font from Google. That is deliberate: the
// alternative is shipping a multi-language fallback font set — megabytes,
// on every load, for every member — to close a rare case. Stated here and
// in the handoff so the limitation is on the record rather than
// discovered later.
//
// ⚠ IF THE APP DOES NOT START, THIS FILE IS THE FIRST SUSPECT. Deleting
// it restores the generated default exactly; see HANDOFF-v56.md, which
// carries the revert as its first paragraph.
...
_flutter.buildConfig = {"engineRevision":"69c8c61792f04cc809dfef0c910414fb9afc06cd","builds":[{"compileTarget":"dart2js","renderer":"canvaskit","mainJsPath":"main.dart.js"},{}],"useLocalCanvasKit":true};


_flutter.loader.load({
  serviceWorkerSettings: {
    serviceWorkerVersion: "2268652197" /* Flutter's service worker is deprecated and will be removed in a future Flutter release. */
  },
  config: {
